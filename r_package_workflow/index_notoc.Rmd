...
Most of the work we will cover here will get you up and running for submission to our GitHub-based platform.  We will use the `devtools` and `usethis` packages to help us with the process of package development.  Change `"/path/to/packages"` to the path where you want the package to be created.  The package itself will be a sub-folder of this folder.   The script will change directories and then install the necessary packages:
