...
To submit a package to `r master_name`, the author/maintainer of the package provides the GitHub URL for the package.  Once the package is submitted several initial checks are conducted.  These checks ensure that the package has been created correctly.  After  initial checks are complete, the package must be verified by email.  This verification is designed to prevent spam and allow the developer to stop a package if they would like to revise the package before re-submitting.

Once the verification is complete, the package is processed.  Overall, the package is copied/cloned to a remote server.  Standardized Travis CI and Appveyor configuration files, specific to `r master_name`, are added.  These are to ensure that the checks performed on these services are consistent for each package.  Some parameters of the package DESCRIPTION file are changed.  These parameters ensure that when a package is downloaded from `r master_name`, the correct versions of the dependent packages are used.  

Next, the package is pushed to the central `r master_name` GitHub (https://github.com/`r project_name`) and submitted to Travis CI and AppVeyor to be built and checked on multiple systems.  Parameters are set to ensure that Travis CI and AppVeyor use the correct versions of `r master_name` packages for checking and external dependencies are installed. The author of the package receives an automatic email indicating whether the package was built successfully and is integrated with `r master_name` together with a description file containing pertinent information about the process. 

## Stable vs. Current Versions

We use the terminology "Stable" and "Current" to differentiate a different status of development for a `r master_name` package.  On the initial submission, after all checks are passed, the package is incorporated into `r master_name` and deemed the Stable version.  The Current version of the package is the result of nightly pulls and mirror the latest package version from the developer's GitHub repository. This provides `r master_name` users with a way to use the latest versions of a package and at the same time it provides the `r master_name` platform with a safe way of checking new versions of a package against the existing set of Current `r master_name` packages. If a Current version of a package passes all the required `r master_name` tests, we contact the developer of the package and suggest an official re-submission to `r master_name`. If the newly re-submitted version of the package passes the checks against the Stable `r master_name` packages, this version is incorporated to the Stable version of `r master_name`.

# The `neuroc.deps` package

We have created the [`neuroc.deps` package](https://github.com/muschellij2/neuroc.deps) that perform most of the backend operations on a `r master_name` package.  It can be installed as follows:

```{r, eval = FALSE}
devtools::install_github("muschellij2/neuroc.deps")
```

The most relevant function is `use_neuroc_template`, which is used to make many of the changes to the package.  For a specific project, you should specify the `user = "`r project_name`"`.

# Changes to the `DESCRIPTION` file
In order to test packages against the relevant `r master_name` packages, we change the `DESCRIPTION` file.  We do this in the following ways:
...
## Travis CI
For Travis CI, we delete the developer's `.travis.yml` configuration script and replace it with the one located at [https://github.com/muschellij2/neuroc.deps/blob/master/inst/`r project_name`_travis.yml](https://github.com/muschellij2/neuroc.deps/blob/master/inst/`r project_name`_travis.yml).
...
looks at the `Remotes` field in the DESCRIPTION file and runs `install_github(..., upgrade_dependencies = FALSE)`.  This ensures that the `r master_name` packages will be those with the specific commit IDs at the time of running.  No old `r master_name` packages will be present as they were removed using `remove_neuroc_packages`.

### PACKAGE_NAME environmental variable

The environmental variable of `PACKAGE_NAME` is created from the `DESCRIPTION` file.  This may be different from the repository name from the user, but will be the same repository name on `r master_name`, as all repos are ``r project_name`/PACKAGE_NAME`.
...
When packages are being deployed, `R CMD INSTALL --build` is run so that they have the standardized naming conventions.  The `deployment` information for `r master_name`, including GitHub encrypted keys, are added.  After building, the binary distribution is uploaded to the GitHub repository when tagged (from `r master_name`'s backend not the developer).
...
### Future work
We plan to add `r master_name` badges to the `README.md` file.  
...
We use the [`r project_name`_appveyor.yml](https://github.com/muschellij2/neuroc.deps/blob/master/inst/`r project_name`_appveyor.yml), which changes the `PATH` variable to try to replicate a Windows machine using Rtools only and not installing MinGW.
