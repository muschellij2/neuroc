...
`preprocess_mri_within` can also perform skull stripping using BET, but we have shown in the [brain extraction tutorial](brain-extraction) that running BET without running neck removal.
...
Similar to the [brain extraction tutorial](brain-extraction), we can run `fslbet_robust` to get a good brain mask for the T1 image.  
