...
```r
source("`r lite_file`")
`r lite_installer`(c("fslr", "neurohcp"))
```
...
```r
`r lite_installer`(c("fslr", "neurohcp"), upgrade_dependencies = FALSE)
```
...
If a package is not in the `r master_name` [list of packages ](`r website`/list-current-packages), then it is not located on the [`r master_name` Github](https://github.com/`r project_name`?tab=repositories).  Therefore, when installing, you'll get the following error:
