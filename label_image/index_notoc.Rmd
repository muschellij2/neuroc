...
Here we will use the N4 corrected, processed images from [Processing Within-Visit MRI](preprocess_mri_within) and register the Eve T1 brain to the processed T1 image.  Let us get the filenames that we want:
...
Now we can register **Eve to the subject T1** (note this is the opposite from [Processing Within-Visit MRI](preprocess_mri_within)).  We will use a `NearestNeighbor` interpolator here.  For the Eve brain image, this may not be a good interpolation and usually we would use a windowed sinc or linear interpolator.  
